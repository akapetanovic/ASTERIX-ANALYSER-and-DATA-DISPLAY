...
{
    public class geStyleMap : geStyleSelector
    {
        public class gePair : geObject
        {
            public geStyleStateEnum Key = geStyleStateEnum.normal;
            public string StyleUrl = "#";

            public gePair(geStyleStateEnum key, string styleUrl)
            {
                Key = key;
                StyleUrl = styleUrl;
            }
            ...
            public override void ToKML(XmlTextWriter kml)
            {
                ...
                kml.WriteElementString("key", Key.ToString());
                kml.WriteElementString("styleUrl", StyleUrl);
                kml.WriteEndElement();
        ...
        public List<gePair> Pairs = new List<gePair>();

        public geStyleMap(gePair pair, string id)
        {
            Pairs.Add(pair);
            ID = id;
        }
